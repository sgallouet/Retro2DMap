...
export function createSampleKingdom(): MapDocument {
  ...
  runSection("structure/castle", [
    ...
    network("castle-wall", [{ x: 16, y: 13 }, { x: 16, y: 14 }]),
    network("castle-wall", [{ x: 37, y: 2 }, { x: 37, y: 18 }]),
    network("castle-wall", [{ x: 17, y: 18 }, { x: 22, y: 18 }]),
  ...
  runSection("actors", [
    ...
    actor("villager-m", 7, 11, "west"),
    actor("guard", 7, 17, "north"),
